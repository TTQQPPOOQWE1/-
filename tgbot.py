...
class Scanner:
        ...
        while not self.stop_event.is_set():
            ...
            with self.lock:
                ...
                if self.checked % 20 == 0:
                    asyncio.run(self.context.bot.send_message(
                        chat_id=self.user_id,
                        text=f"🔄 Прогресс: {self.checked}/{self.total}\nОткрытых: {self.open_count}, Успешных: {self.success_count}"
                    ))
    ...
    def run(self):
        ...
        asyncio.run(self.context.bot.send_message(
            chat_id=self.user_id,
            text=f"🏁 Сканирование завершено!\nIP всего: {self.total}\nОткрытых портов: {self.open_count}\nКамер найдено: {self.success_count}"
        ))
