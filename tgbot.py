...
async def process_xml_file(update: Update, content: str):
    try:
        ...
        # Определяем конфигурацию генерации с увеличенным лимитом
        generation_config = genai.types.GenerationConfig(
            max_output_tokens=600000  # Максимальный лимит для gemini-1.0-pro
        )
...
---
"""
        # Передаем конфигурацию в вызов API
        ...
        # Дополнительная проверка на случай, если ответ все равно обрезан
        if not generated_xml.endswith('</Organization>'):
            logger.warning("Ответ от Gemini API может быть обрезан, так как не заканчивается на </Organization>")
            # Здесь можно либо сообщить пользователю об ошибке, либо попробовать "дочинить" XML,
            # но лучше просто убедиться, что лимита хватает.
        ...
        # Полезно посмотреть на 'response.prompt_feedback' если ошибка связана с безопасностью
...
def worker(self):
    while not self.stop_event.is_set():
        ...
        # Печатаем текущий IP и статистику в консоль
        print(f"[SCAN] Checking {ip}... | OPEN: {self.open_count} | SUCCESS: {self.success_count}")
        ...
        for port in ports:
            if check_port(ip, port):
                ...
                if ok:
                    ...
                    break  # не проверяем другие порты, если успех
